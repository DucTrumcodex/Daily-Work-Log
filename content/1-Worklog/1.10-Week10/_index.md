---
title: "Week 10 Worklog"
date: 2026-07-06
weight: 10
chapter: false
pre: " <b> 1.10. </b> "
---



### Week 10 Objectives:

* Continue project development: refine Avatar and Shop API/flows.
* Integrate Amazon EventBridge to automate project tasks.
* Test new flows and fix issues found.
* Update docs + architecture diagram based on what was built.

### Tasks to be carried out this week:
| Day | Task | Start Date | Completion Date | Reference Material |
| --- | ---- | ---------- | --------------- | ------------------ |
| 2 | - Refine Avatar API/flow in the project <br> - Review S3/CloudFront permissions in use | 06/07/2026 | 06/07/2026 | - |
| 3 | - Design EventBridge use cases for the project <br> - Create rules/targets for automation | 07/07/2026 | 07/07/2026 | - |
| 4 | - Integrate EventBridge into the project flow <br> - Test triggers and processing results | 08/07/2026 | 08/07/2026 | - |
| 5 | - Fix Avatar/Shop/EventBridge bugs <br> - Optimize retry/timeout if needed | 09/07/2026 | 09/07/2026 | - |
| 6 | - Update the project architecture diagram <br> - Note the EventBridge configuration deployed | 10/07/2026 | 10/07/2026 | - |


### Week 10 Achievements:

* Project development:
  * Further refined the Avatar flow and related Shop parts within the week’s scope.
  * Integrated EventBridge for at least one automated project task.

* Testing & documentation:
  * Tested event triggers, logged issues, and fixed main bugs.
  * Updated the diagram/docs to reflect EventBridge added to the project.
